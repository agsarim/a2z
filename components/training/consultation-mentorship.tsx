...
import { Users, Calendar, MessageSquare, CheckCircle, ArrowRight } from "lucide-react"
      ...
      <div className="container mx-auto px-4">
        ...
        >
          ...
          <h2 className="text-4xl lg:text-5xl font-bold text-slate-900 mb-6">
            Personalized Guidance for <span className="text-blue-600">Accelerated Growth</span>
          </h2>
          <p className="text-xl text-slate-600 max-w-3xl mx-auto">
            For authors seeking dedicated support and strategic guidance, our mentorship programs provide direct access
            to expert insights and personalized action plans.
          </p>
        </motion.div>
              ...
              >
                <CardContent className="p-8">
                  <div className="text-center space-y-6">
                    ...
                    >
                      {index === 0 ? <Calendar className="w-8 h-8" /> : <MessageSquare className="w-8 h-8" />}
                    </div>
                    <h3 className={`text-2xl font-bold ${program.highlight ? "text-white" : "text-slate-900"} mb-2`}>
                      {program.name}
                    </h3>
                    ...
                    <div className="text-left mt-6 space-y-4">
                      <h4 className={`text-lg font-semibold ${program.highlight ? "text-white" : "text-slate-800"}`}>
                        What's Included:
                      </h4>
                      <ul className="space-y-3">
                        {program.included.map((item, itemIndex) => (
                          <li key={itemIndex} className="flex items-start gap-3">
                            ...
                            />
                            <span className={`${program.highlight ? "text-blue-100" : "text-slate-700"}`}>{item}</span>
                          </li>
                        ))}
                      </ul>
                    </div>
